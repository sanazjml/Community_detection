    ...
    for j=1:length(NB)
      ...
      if (G(Cj) == 0)
        ...
        if G(Cj) > best_increase;
          best_increase = G(Cj);
          Cnew_t = Cj;         %shomare khosheee ke behtarin modul ra darad ham save mikonad
        end 
      end
    end
    if best_increase > 0
      Cnew = Cnew_t;
      if (debug)
        fprintf('Move %d => %d\n',i-1,Cnew-1);   %%indices start from zero
      end
      Cost(i) = best_increase;   %% behtarin modularity ke har khooshe darad ra save mikonad
